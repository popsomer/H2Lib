...
//  tst = new_avector(n);//Ax-b iso rhat
//  norb = norm2_avector(b); 
...
//      copy_avector(b,tst);
//printf("bnor=%g, x=%g\n", norm2_avector(tst),norm2_avector(x) );
//      addevalA(-1.0, A, x, tst);
//printf("axb=%g, x= %g\n", norm2_avector(tst),norm2_avector(x) );
      //addeval_amatrix_avector(-1.0, Vfull, xf, tst);
//      axmb = norm2_avector(tst);
      if(false) {
	printf("  Residual: %.5e\t Iterations: %u\r", norm, j + i);
	fflush(stdout);
      }
      if (norm <= accuracy) {
//        printf("norm: %g <= %g. kk=%i, i= %i, j= %i, |Ax-b|= %g, |b|=%g \n", norm, accuracy, kk,i,j, axmb,norb);
	finish_gmres(addevalA, A, b, x, rhat, q, &kk, qr, tau);
	break;
      }
    }
    if (norm <= accuracy) {
//      printf("Reached accuracy: %g <= %g. kk=%i, i= %i, |Ax-b|= %g, |b|=%g \n", norm, accuracy, kk,i,axmb,norb);
      break;
    }
    else {
      finish_gmres(addevalA, A, b, x, rhat, q, &kk, qr, tau);
